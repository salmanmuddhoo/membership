...
| `DATABASE_POOL_MAX`               | Per-instance connection pool ceiling                                                                                      | No      | Set `10` on App Service: one long-lived process, so this is the whole app's ceiling (`docs/database.md`)                                                                                  |
...
| `PUBLIC_APP_URL`                  | The address links the Society sends open on (a member's receipt)                                                          | No      | Defaults to the origin of `ENTRA_REDIRECT_URI`; set it only when the app is reached at another address                                                                                    |
| `MEMBER_SESSION_SECRET`           | Signs the member mobile app's access tokens — its own secret, never `AUTH_SESSION_SECRET`                                 | **Yes** | Key Vault reference; `openssl rand -base64 40`, at least 32 characters. Rotating signs out every member app user and expires every receipt link sent.                                     |
